...
- [ ] Chris: wire `ReservationForm.jsx`'s `handleSubmit` (currently just `console.log`) to `POST /api/reservations` — see `docs/API-CONTRACT.md` for the actual response shape (no `success` key yet).
